...
```
Save animation and re load into file: 
...
```
```{r, eval=FALSE}
anim_save("garden_harvest2.gif") 

knitr::include_graphics("garden_harvest2.gif")
```
...
animate(mallorca_1, duration = 20, end_pause = 7)
```
...
geom_point(mapping = aes(x = lon, 
             ...
             color = event, 
             size = 2.5) + 
scale_color_manual(values = c("Bike" = "darkblue",
                                "Run" = "darkgreen",
                                "Swim" = "lightpurple")) + 
labs(x = "",
     y = "",
